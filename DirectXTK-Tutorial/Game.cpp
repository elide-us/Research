...
using namespace VoxelFarm;           // VoxelFarm.h
using namespace VoxelFarm::Algebra;  // VoxelFarm.h
using namespace VoxelFarm::Physics;  // VoxelFarm.h
using namespace VoxelFarm::IO;       // VoxelDB.h
...
void Game::Render()
{
	...
	//HACK: Can be used to apply alpha blending to non-PMA graphics
	...
	m_font->DrawString(m_spriteBatch.get(),
		...
		0.0f,              // (Rotation) ?
		origin             // Center
		// Assuming there's a scaling overload as well
	);
...
//		cosf(time) * 4.0f,
		m_origin            // Center
//		m_origin,
//		cosf(time + 2.0f)   // Overload option adds Scaling
	);
...
#pragma endregion
	...
	m_fontPos.x = backBufferWidth / 4.0f;
	m_fontPos.y = backBufferHeight / 4.0f;
}
